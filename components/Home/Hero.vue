<template>
  <div class="flex gap-12 justify-center items-center">
    <div class="max-w-xl prose prose-lg">
      <h1 class="">Embed a private video course on any website.</h1>
      <p>
        No server code needed, just simple JavaScript you can add to any site
        for a full-featured, paywalled video course.
      </p>
    </div>
    <div
      class="relative sm:max-w-lg sm:mx-auto lg:mt-0 lg:max-w-none lg:mx-0 lg:col-span-6 lg:flex lg:items-center"
    >
      <div class="relative mx-auto w-full lg:max-w-md">
        <img class="w-full" src="/coursekit_homepage.png" alt="CourseKit" />
      </div>
    </div>
  </div>
</template>
